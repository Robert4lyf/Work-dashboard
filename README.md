...
- **Interruptions:** during a focus session, tap **Interrupted** to log one (it pauses the timer so you can deal with it); add what it was, or pick a reason you've used before. History shows the last 7 days: how many, how often per focus hour, the main causes and the busiest hour.
...
- **Keyboard shortcuts (computer):** `i` or `n` capture to the inbox (`n` on a quest's page adds a subquest), `t` today, `f` focus, `l` history, `s` settings, `z` single-task, `p` pause/resume the timer, `Esc` back, `?` show these.
