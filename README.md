...
- **Estimates:** on a quest (or a step), pick 15m, 30m, 1h or 2h. Today then shows what's planned against the free time left: from now to the end of your workday (Settings > Appearance, 17:30 by default), minus the meetings still to come. When it doesn't fit, the line turns orange. Waiting quests don't count.
...
- **Header:** your next step (tick it off, tap to open it, or **Focus** to start single-task mode), today's progress, focus time and meeting status. While a focus timer runs, it shows the countdown with a pause button. Anything needing attention (overdue, due today, to chase, weekly review) shows at the top of Today instead, and the Waiting and Review tabs get a badge.
...
- **Talk mode (optional, per device):** turn it on in **Settings > Talk mode** and a **Talk** button appears in the header (there's also a **Talk** app-icon shortcut). The app reads out your day (time, next meeting, planned vs free, Next up, inbox, anything to chase) and listens for an answer: "done", "start focus", "tomorrow", "waiting on Sam", "add …" (to the inbox), "inbox" (goes through it item by item: today, tomorrow, next week, delete or skip), "meetings", "next", "repeat", "help" and "stop". Works with headphones; the screen stays on while it's open, since Android stops listening when the screen locks.
