...
- **Waiting:** on a quest (or a step), open **Waiting on someone?** to say who and what for, with an optional date to chase. It stays on Today with a "Waiting on …" tag but isn't Next up. The Waiting tab lists everything you're waiting on, lets you add a new one (it lands in the Inbox, already waiting), and **Got it** puts it back on your list. Chase dates show in the header and get a 9am notification. A quest with a waiting step shows as waiting too ("Step waiting on Sam"); its other steps carry on. Waiting quests sit at the bottom of Today (above finished ones) and move back to their place when the wait is over.
