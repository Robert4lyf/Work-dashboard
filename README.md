...
- **Projects:** the Projects tab is where you set up projects and see each one's finished and open quests, progress and focus time. Add a quest to a project right there (it goes on Today), or pull in one that's already on Today. You can also pick a project on a quest or an inbox item. Finish, rename or delete projects from the tab.
...
- **Waiting:** on a quest (or a step), open **Waiting on someone?** to say who and what for, with an optional date to chase. It stays on Today with a "Waiting on …" tag but isn't Next up. The Waiting tab lists everything you're waiting on, lets you add a new one, and **Got it** puts it back on your list. Chase dates show in the header and get a 9am notification.
- **Interruptions:** during a focus session, tap **Interrupted** to log one; add what it was, or pick a reason you've used before. History shows the last 7 days: how many, how often per focus hour, the main causes and the busiest hour.
